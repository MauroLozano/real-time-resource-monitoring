...
export const getStaticData = async (): Promise<StaticData> => {
  const [rawCpuData, rawMemoryData, rawMemoryLayoutData, rawStorageData] =
    await Promise.all([si.cpu(), si.mem(), si.memLayout(), si.diskLayout()]);
  const staticData: StaticData = {
    ...
    memory: {
      total: rawMemoryData.total
        ? parseFloat((rawMemoryData.total / 1024 ** 3).toFixed(2))
        : null,
