...
export async function getWmiTemperature(): Promise<WmiCpuTempResponse> {
  ...
  try {
    ...
    const packageSensor = data.find(
      (entry) => entry.Name.includes('Tctl') || entry.Name.includes('Package')
    );
    const packageTemp = packageSensor
      ? formattedFloat(packageSensor.Value)
      : null;
