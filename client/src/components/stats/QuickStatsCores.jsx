...
export default function QuickStatsCores(){
    const {currentData, tempData, computedStats, maxValues, sensorsHealth } = useMetrics()
    ...
    return (
        <aside className={styles.quickStats}>
            <QuickStatsElement title ='Core Max. Temp' label={`Core ${coreMaxTemp.index?? '-'}`} data={coreMaxTemp.value} id={metricsIds.CORE_MAX_TEMP} reason={sensorsHealth?.cpu?.temp?.reason} unit='°C'/>
            ...
            <QuickStatsElement title ='Core Overload' data={coreOverload} id={metricsIds.CORE_OVERLOAD} unit='%'/>
            <QuickStatsElement title ='Most Active Core' label={`Core ${mostActiveCore.coreNumber?? '-'}`} data={mostActiveCore.value} id={metricsIds.MOST_ACTIVE_CORE} unit='%'/>
            <QuickStatsElement title ='Parked Cores' data={`${parkedCores}/${totalThreads}`} id={metricsIds.PARKED_CORES}/>
            <QuickStatsElement title ='Thread Efficiency' label={`${threadEfficiency ? `${activeThreadsCount}/${totalThreads}` : '-'}`} data={threadEfficiency} id={metricsIds.THREAD_EFFICIENCY} unit='%'/>
        </aside>
    )
}