...
export const MetricsProvider = ({children})=>{
    ...
    const coreMaxTemp = useMemo(()=>{
        if (!tempData?.coresTemp || tempData.coresTemp.length === 0) return {index: null, value: null}
    ...
    useEffect(()=>{
        socket.on('dynamicData', (data)=>{
            ...
            if (!hasError){
                ...
                setMaxValues((prev) =>{
                    return getMaxValues(prev, {cpuLoad: data.cpuLoad, coresSpeed: data.cpuSpeed?.cores || []})
                })
            }else{
